<?php
require_once __DIR__ . '/../models/OptionSimpleMultiple.php';

class OptionSimpleMultipleController
{
    // Listar
    public static function getAll()
    {
        try {
            $model = new OptionSimpleMultiple();
            $items = $model->obtenerTodos();

            http_response_code(200);
            echo json_encode($items);
        } catch (Exception $e) {
            self::sendError(500, "Error al obtener los registros de option_simple_multiple.", $e);
        }
    }
    ...
    // Obtener por id_simple_multiple
    public static function getBySimpleMultiple($id_simple_multiple)
    {
        if (!is_numeric($id_simple_multiple)) {
            return self::sendError(400, "El parámetro 'id_simple_multiple' debe ser numérico.");
        }

        try {
            $model = new OptionSimpleMultiple();
            $items = $model->obtenerPorIdSimpleMultiple($id_simple_multiple);

            http_response_code(200);
            echo json_encode($items);
        } catch (Exception $e) {
            self::sendError(500, "Error al obtener opciones por id_simple_multiple.", $e);
        }
    }
    ...
    public static function create($data)
    {
        ...
        if (!isset($data['id_simple_multiple']) || !is_numeric($data['id_simple_multiple'])) {
            http_response_code(400);
            echo json_encode(["error" => "El campo 'id_simple_multiple' es obligatorio y debe ser numérico"]);
            return;
        }
        ...
        try {
            ...
            $creado = $model->crear([
                ...
                'id_simple_multiple' => (int)$data['id_simple_multiple'],
    ...
    public static function update($id, $data)
    {
        ...
        if (array_key_exists('id_simple_multiple', $data)) {
            if (!is_numeric($data['id_simple_multiple'])) {
                return self::sendError(400, "El campo 'id_simple_multiple' debe ser numérico.");
            }
            $payload['id_simple_multiple'] = (int)$data['id_simple_multiple'];
        }
    ...
    public static function truncate()
    {
        try {
            ...
            http_response_code(200);
            echo json_encode(["message" => "Tabla option_simple_multiple vaciada exitosamente."]);
        } catch (Exception $e) {
            self::sendError(500, "Error al vaciar la tabla option_simple_multiple.", $e);
        }
    }
