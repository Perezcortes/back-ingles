<?php
require_once __DIR__ . '/../models/QuestionNumber.php';
...
class QuestionNumberController
{
    ...
    {
        try {
            $model = new QuestionNumber();
            $items = $model->obtenerTodos();
            http_response_code(200);
            echo json_encode($items);
        } catch (Exception $e) {
            self::sendError(500, "Error al obtener los registros de question_number.", $e);
        }
    }
    ...
    // Obtener por id_multiple_choice_cloze (activos)
    public static function getByMultipleChoiceCloze($id_multiple_choice_cloze)
    {
        if (!is_numeric($id_multiple_choice_cloze)) {
            return self::sendError(400, "El id_multiple_choice_cloze debe ser numérico.");
        }

        try {
            $model = new QuestionNumber();
            $items = $model->obtenerPorMultipleChoiceCloze($id_multiple_choice_cloze);
            http_response_code(200);
            echo json_encode($items);
        } catch (Exception $e) {
            self::sendError(500, "Error al obtener por id_multiple_choice_cloze.", $e);
        }
    }
    ...
    {
        if (!isset($data['id_multiple_choice_cloze']) || !is_numeric($data['id_multiple_choice_cloze'])) {
            http_response_code(400);
            echo json_encode(["error" => "El campo 'id_multiple_choice_cloze' es obligatorio y debe ser numérico"]);
            return;
        }
        ...
        try {
            $model = new QuestionNumber();
            $creado = $model->crear([
                'id_multiple_choice_cloze' => (int)$data['id_multiple_choice_cloze'],
            ]);
    ...
    public static function update($id, $data)
    {
        ...
        if (array_key_exists('id_multiple_choice_cloze', $data)) {
            if (!is_numeric($data['id_multiple_choice_cloze'])) {
                return self::sendError(400, "El campo 'id_multiple_choice_cloze' debe ser numérico.");
            }
            $payload['id_multiple_choice_cloze'] = (int)$data['id_multiple_choice_cloze'];
        }
    ...
    public static function truncate()
    {
        try {
            ...
            $model->vaciarTabla();
            http_response_code(200);
            echo json_encode(["message" => "Tabla question_number vaciada exitosamente."]);
        } catch (Exception $e) {
            self::sendError(500, "Error al vaciar la tabla question_number.", $e);
        }
    }
